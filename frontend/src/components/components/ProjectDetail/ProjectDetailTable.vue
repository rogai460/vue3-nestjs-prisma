...
<template>
  <h5 class="py-4 font-bold uppercase text-gray-600 dark:text-white">{{ projectInfo?.projectName }} 詳細</h5>
  <!--Table Card-->
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
      ...
      <tbody>
        <tr v-for="dt in tableData" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
          <td scope="col" class="py-3 px-6">{{ dt.lastName }} {{ dt.firstName }}</td>
          <td scope="col" class="py-3 px-6">{{ dt.lastNameKana }} {{ dt.firstNameKana }}</td>
          ...
          <td class="text-center">
            <label class="inline-flex relative items-center cursor-pointer">
              <input v-model="dataIds" type="checkbox" class="sr-only peer" :value="dt.id" @change="notExists" />
              <div
      ...
      <tfoot>
        <tr class="font-semibold text-gray-900 dark:text-white">
          ...
          <td class="py-3 px-6"></td>
          <td class="py-3 px-6"></td>
          <td class="py-3 px-6 text-right">¥ {{ sumSales.toLocaleString() }}</td>
          <td class="py-3 px-6 text-right">¥ {{ sumCost.toLocaleString() }}</td>
          <td class="py-3 px-6 text-right">¥ {{ sumProfit.toLocaleString() }}</td>
