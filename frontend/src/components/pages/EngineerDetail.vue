...
const projectHistoryTableData = computed(() => (engineerResponse.value ? engineerResponse.value.projectHistory : []));

const postProjectHistory = async (projectHistoryInput: ProjectHistoryPostInput) => {
  await createProjectHistory(projectHistoryInput);
  reloadEngineerResponse();
};

const reloadEngineerResponse = async () => {
  engineerResponse.value = await getEngineerWithHistory(String(route.query.engineerId));
};
...
<template>
  <ProjectHistoryTable
    :engineer-id="Number($route.query.engineerId)"
    :engineer-data="engineerData"
    :table-data="projectHistoryTableData"
